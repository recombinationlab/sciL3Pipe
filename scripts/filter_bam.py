...
def plot_soft_clipped(soft_clipped, read='', ratio_filt=False):
    ...
    if ratio_filt:
        ...
        plt.hist(soft_clipped, bins=max([max(i) for i in soft_clipped]), 
        density=False, histtype='stepfilled',
        stacked=True, label=['Pass', 'Fail'])
...
def filter_paired_reads(args):
        ...
        for read, mate in read_pair_generator(input_bam):
            ...
            if all([not read.is_unmapped,
                ...
                if all([not read.is_unmapped,
                        not mate.is_unmapped,
                        valid_ratio]):
                    soft_clip_ratio_pass_r1.append(soft_clipped(read.cigarstring))
                    soft_clip_ratio_pass_r2.append(soft_clipped(mate.cigarstring))
                elif all([not read.is_unmapped,
                            not mate.is_unmapped]):
                        soft_clip_ratio_fail_r1.append(soft_clipped(read.cigarstring))
                        soft_clip_ratio_fail_r2.append(soft_clipped(mate.cigarstring))
    ...
    if not args.no_plot:
        ...
        p5 = plot_soft_clipped(np.array([soft_clip_pass_r1, soft_clip_fail_r1], dtype=object),
                               read='read 1')

        p6 = plot_soft_clipped(np.array([soft_clip_pass_r2, soft_clip_fail_r2], dtype=object),
                               read='read 2')

        p7 = plot_soft_clipped(np.array([soft_clip_ratio_pass_r1, soft_clip_ratio_fail_r1], dtype=object), 
                               read = 'read 1', ratio_filt=True)

        p8 = plot_soft_clipped(np.array([soft_clip_ratio_pass_r2, soft_clip_ratio_fail_r2], dtype=object), 
                               read = 'read 2', ratio_filt=True)

        p9 = plot_soft_clipped(np.array([np.array(soft_clip_pass_r1) + np.array(soft_clip_pass_r2), 
                                         np.array(soft_clip_fail_r1) + np.array(soft_clip_fail_r2)], dtype=object),
                               read='read 1 and 2')

        p10 = plot_soft_clipped(np.array([np.array(soft_clip_ratio_pass_r1) + np.array(soft_clip_ratio_pass_r2), 
                                         np.array(soft_clip_ratio_fail_r1) + np.array(soft_clip_ratio_fail_r2)], dtype=object), 
                               read = 'read 1 and 2', ratio_filt=True)

        p11 = soft_clipped_vs_matched(np.array(sc_r1) + np.array(sc_r2), 
                                     np.array(m_r1) + np.array(m_r2), args.max_ratio)

        p12 = matched_vs_matched(m_r1, m_r2)

        write_html_report(html_out, [p1, p2, p3, p4, p5, p6, p7, p8, p9, p10, p11, p12])
