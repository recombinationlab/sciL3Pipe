...
def SSS_barcode_extract(r1, r2, output_folder, barcode_dict, RT_primer, 
    ...
    with open(noRT_R1_out_file, 'w') as noRT_R1, \
        ...
        for s1, s2 in zip(pyfastx.Fastq(r1, build_index=False, full_name=True), 
                        pyfastx.Fastq(r2, build_index=False, full_name=True)):
            ...
            if dist_rt_r1 <= mismatch_RT:
                ...
                if barcode:
                    UMI = seq1[:4]
                    name1_new = '@' + barcode + ',' + UMI + ',' + name1
                    name2_new = '@' + barcode + ',' + UMI + ',' + name2
                    # split by SSS
                    try:
            ...
            # if RT primer found in R2, swap R1 R2 but keep read name
                ...
                if barcode:
                    UMI = seq1[:4]
                    name1_new = '@' + barcode + ',' + UMI + ',swap,' + name1
                    name2_new = '@' + barcode + ',' + UMI + ',swap,' + name2
                    # swap
                    # split by SSS
                    try:
